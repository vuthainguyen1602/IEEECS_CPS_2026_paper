...
Để giải quyết vấn đề này, SMOTEENN \cite{batista2004study} trước tiên áp dụng SMOTE để xây dựng một lượng mẫu thiểu số tổng hợp cân bằng. Sau đó, Edited Nearest Neighbors (ENN) \cite{wilson1972asymptotic} đóng vai trò như một cơ chế làm sạch dữ liệu nâng cao. Nó đánh giá vùng lân cận của mỗi mẫu bằng quy tắc $k$-lân cận gần nhất; nếu đa số các lân cận của một mẫu thuộc về một lớp khác, mẫu đó được coi là nhiễu và bị loại bỏ. Phương pháp tiếp cận hai giai đoạn này xây dựng lại một cách mạnh mẽ một không gian vectơ huấn luyện $S_{train}$ được căn chỉnh và không có nhiễu, nơi xác suất tiên nghiệm $P(\text{Class} = k)$ được cân bằng và ranh giới quyết định vẫn duy trì được sự sắc nét.
...
\tcc{Bước 3.1: Sinh dữ liệu SMOTE}
